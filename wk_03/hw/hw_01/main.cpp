...
int main()
{
    ...
    while ( processBook )
    {
        ...
        string line;
        char selection       = 'n';
        char alphebetL[ 26 ] = { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i',
                                 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r',
                                 's', 't', 'u', 'v', 'w', 'x', 'y', 'z' };

        for ( int k{}; k < 26; k++ )
            letterCount[ k ] = 0; // initialize all values in letterCount to 0

        data >> inFile;
        data >> word;
        book.title = inFile + " " + word; // Finding title
        data >> inFile;
        data >> word;
        book.author = inFile + " " + word;
        ...
        {
            if ( !( inFile.empty() || inFile == "Contents: " ||
                    inFile == "Contents:" ) )
            {
                book.lineCount++;
        ...
        book.wordCount = wordCount + book.lineCount +
                         1; // add number of lines to account for new lines
        ...
        for ( int k = 0; k < 26; k++ )
        {
            book.letterFrequency[ k ] =
                static_cast<double>( letterCount[ k ] ) / totalLetters;
        }
        ...
        if ( selection == 'y' || selection == 'Y' )
        {
            ...
            for ( int i = 0; i < 26; i++ )
            {
                cout << alphebetL[ i ] << ": " << book.letterFrequency[ i ]
                     << endl;
            }
        }
