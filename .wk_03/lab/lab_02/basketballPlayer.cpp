...
basketballPlayer::basketballPlayer( std::string Name, int ShotsTaken,
                                    int ShotsMade, int PassesAttempted,
                                    int PassesMade )
{
    this->Name            = Name;
...
bool basketballPlayer::PassBall()
{
    ...
    double passingPercentage =
        100 * static_cast<double>( PassesMade ) / PassesAttempted;

    PassesAttempted++;
    if ( PassesMade == 0 ) // This is used once to initialize PassesMade
                           // otherwise it always is 0
    {
        if ( passChance > 100 / 2 )
        {
            PassesMade++;
...
int basketballPlayer::TakeShot( int points )
{
    double shootingPercentage =
        100 * static_cast<double>( ShotsMade ) / ShotsTaken;
    ...
    ShotsTaken++;
    if ( ShotsMade ==
         0 ) // value will always be zero if there isn't a special case checking
    {
        if ( points == 3 )
        {
